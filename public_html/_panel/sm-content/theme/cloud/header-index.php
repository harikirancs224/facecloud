				...
				<fieldset style="margin-left:12px;">
					...
					<div class="col-md-3 property">
						...
						<select name="category" class="">
							...
							<?php 
								c_options_list(CATID,0,'');
								// echo options('0','category','','active') 
							?>
						</select>
					  </div>
...
<div class="container desk">
	<?php echo do_shortcode('[BSLIDER height=340px]'); ?>
</div>
<div class="clr "></div>
<div class="clr "></div>
		...
		<form action="<?php bloginfo("siteurl"); ?>/search/" method="get">
			<fieldset>
				...
				<!-- <div class="col-md-3 gender">
					  <p>Gender:</p>
					  <label for="malemob">
						<input <?php //if(isset($_GET['gender'])&&$_GET['gender']=="male") echo 'checked';?> type="radio" value="male" name="gender" id="malemob" checked > <span style="float:left;">Male</span>
					  </label>

					  <label for="femalemob">
						<input <?php //if(isset($_GET['gender'])&&$_GET['gender']=="female") echo 'checked';?> type="radio" value="female"  name="gender" id="femalemob"> <span>Female</span>
					  </label>
				  </div> -->
				  <div class="col-md-3 property">
					  <p>Search Type</p>
...
$(function(){
	...
	if(lc!="nulld" && lc!=dlc)
	{
		$("#locatorin").val(lc);//alert("ifpart "+lc);
	}
	else{
		$("#locatorin").val(dlc);//alert("elsepart");
	}
});
