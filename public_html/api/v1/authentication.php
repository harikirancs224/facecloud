...
$app->post('/login', function() use ($app) {
	...
	$c=substr(str_shuffle($chars),0,8);
	
	$to=$user['email'];
	$subject="Step2 Verification";
	$msg="Dear Facecloud User, <br/> Thanks for Requesting Verification Code.<br/>Your Verification Code is : <b>".$c."";
...
$app->post('/signUpModify', function() use ($app) {
    $response = array();
    $r = json_decode($app->request->getBody());
	...
	$session = $db->getSession();
	$uid=$session["uid"];
	$result = $db->query("update ". $tabble_name." set name='".$name."',email='".$email."',address='".$address."',phone='".$phone."' where uid='".$uid."'");
...
$app->post('/passwordModify', function() use ($app) {
    $response = array();
    $r = json_decode($app->request->getBody());
    verifyRequiredParams(array('new', 'current', 'confirm'),$r->customer);
    require_once 'passwordHash.php';
    $db = new DbHandler();
    $password = $r->customer->new;
    $confirm = $r->customer->confirm;
    $current = $r->customer->current;
    $address = $r->customer->address;
	...
	$user = $db->getOneRecord("select uid,name,password,email,created from customers_auth where uid='$uid'");
	if($confirm!=$password){
		$response['status'] = "error";
		$response['message'] = 'New password and Confirm password not matched!';
	}
    else if ($user != NULL) {
        if(passwordHash::check_password($user['password'],$current)){
			try{
				$confirm=passwordHash::hash($confirm);
				$result = $db->query("update ". $tabble_name." set password='".$confirm."' where uid='".$uid."'");
				//send_mail($to,$subject,$msg,$fromname);
